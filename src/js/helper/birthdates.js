...
export const ageInMonths = date => {
    parseDate(date);
};

export const isBaby = birthdate => {
    const today = moment();
    return (
        moment(birthdate)
            .add(12, "month")
...
export const isToddler = birthdate => {
    const today = moment();
    return (
        moment(birthdate)
            .add(12, "month")
...
export const getChildrenTypesMap = member => {
    const types = {};
...
const getAgeString = attributes => {
    ...
    if (weeks < 4) {
        return { weeks, age: "newborn", type: BIRTH_TYPE.baby, slug: "0-month-old-baby" };
    }
    ...
    return {
        weeks,
        age: ageString,
        type: BIRTH_TYPE.toddle,
        slug: `${getTotalMonth(years, months)}-month-old-month-old`,
    };
};
...
const getPregnancyInfo = pregnancy => {
    ...
    return {
        type: BIRTH_TYPE.pregnancy,
        age: parsedBrithDay,
        weeks: currentWeek,
        slug: `${currentWeek}-weeks-pregnant`,
    };
};

export const getChildrenMapping = member => {
...
export const getChildrenInfo = childrenMapping => {
    return [].concat(
        childrenMapping[BIRTH_TYPE.pregnancy],
        childrenMapping[BIRTH_TYPE.baby],
        childrenMapping[BIRTH_TYPE.toddle],
    );
};
...
const filterToddleIrregular = (children, type) => {
    ...
    if (minToddle) {
        ...
        if (getTotalMonth(year, month) <= MAX_MONTHS) {
            return minToddle;
        }
    }
    return null;
...
export const getStuffChildren = childrenMapping => {
    const data = [];
